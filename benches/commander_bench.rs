...
fn bench_git_fake_status(c: &mut Criterion) {
    let (_tmp, _base) = create_temp_dir_with_files(100, true);
...
fn bench_natural_sort(c: &mut Criterion) {
    let names: Vec<String> = (0..500)
        .map(|i| format!("file{}.txt", i % 73 + (i / 7)))
        .collect();
                ...
                while i < aa.len() && j < bb.len() {
                    if aa[i].is_ascii_digit() && bb[j].is_ascii_digit() {
                        ...
                        while i < aa.len() && aa[i].is_ascii_digit() {
                            na = na * 10 + (aa[i] - b'0') as u64;
                            i += 1;
                        }
                        while j < bb.len() && bb[j].is_ascii_digit() {
                            nb = nb * 10 + (bb[j] - b'0') as u64;
                            j += 1;
                        }
                        if na != nb {
                            return na.cmp(&nb);
...
criterion_group!(
    benches,
    bench_temp_setup,
    bench_git_fake_status,
    bench_tab_duplicate,
    bench_natural_sort,
    bench_parse_and_match_mask,
    bench_format_size
);
